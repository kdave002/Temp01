...
- 2026-03-02T05:58:00Z: Extended rate-limit tests to verify response introspection headers (`X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`) on successful protected POST calls and on 429 responses (with `Retry-After`); local run `.venv/bin/python -m pytest -q` passed (53/53). [UTC DONE]
...
- 2026-03-03T17:31:00Z: Added schema-aware patch generation regression tests for rename projection (`old AS new`) and type-change casting (`SAFE_CAST` to BigQuery type); full suite + compile checks passed (`pytest -q`: 55/55, `make check`: pass). [UTC DONE]
